    ...
    fun BrandDetailScreen(
        ...
        userViewModel: UserViewModel = hiltViewModel()// Injected ViewModel
    ) {
        ...
        val averageRatingState by userViewModel.averageRatingState.collectAsState()

        LaunchedEffect(brandId) {
            userViewModel.getAverageRatingForBrand(brandId)
        }
        ...
        Column(modifier = modifier.fillMaxSize().padding(dimensionResource(id = R.dimen.dimen_16_dp))) {
            ...
            when (averageRatingState) {
                is UiState.Success -> {
                    val rating = (averageRatingState as UiState.Success).data
                    ...
                    Text("Based on reviews", fontSize = 16.sp)
                }
